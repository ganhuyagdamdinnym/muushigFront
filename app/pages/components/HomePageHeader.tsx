...
export const HomePageHeader = () => {
  ...
  // Portal-д ашиглахын тулд client-side mount хүлээх
  useEffect(() => {
    setMounted(true);
  }, []);
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 h-14 sm:h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-800 px-3 sm:px-6 flex items-center justify-between z-30 select-none">
        {/* Logo */}
        <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
          ...
          <span className="hidden sm:inline text-[9px] sm:text-[10px] bg-slate-800 text-slate-400 px-1.5 py-0.5 rounded uppercase font-semibold tracking-widest">
            v2.0
          </span>
        </div>
      ...
      {/* Sidebar — document.body руу portal хийснээр header-ийн гадна render болно */}
      {mounted &&
        sideBarOpen &&
        createPortal(
          <SideBar
